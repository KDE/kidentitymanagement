...
KIdentityManagement::Identity CryptographyEditorBackend::identity() const
{
    return m_cryptoBackend->identity();
}

void CryptographyEditorBackend::setIdentity(const KIdentityManagement::Identity &identity)
{
...
QModelIndex
CryptographyEditorBackend::indexForIdentity(QAbstractItemModel *model, const KIdentityManagement::Identity &identity, const KeyUseTypes::KeyUse keyUse)
{
    Q_ASSERT(model);
